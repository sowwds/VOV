<template>
    <div class="fixed bg-[#fafafc] dark:bg-[#17181c] inset-0 -z-10 overflow-hidden">
      <canvas
          ...
          :class="{'opacity-50': isContentPage, 'opacity-0': !isVisualizerActive}"
      ></canvas>
    </div>
  </template>
  ...
  import { useVisualizerStore } from '@/store/visualizer'; // Import the visualizer store

  const playerStore = usePlayerStore();
  const isPlaying = computed(() => playerStore.isPlaying);
  const audioElement = computed(() => playerStore.audioElement);
  const route = useRoute();
  const visualizerStore = useVisualizerStore(); // Use the visualizer store

  const isContentPage = computed(() => route.path !== '/');
  const isAnimationEnabled = computed(() => visualizerStore.isAnimationEnabled); // Get animation state from store
  const shouldAnimate = computed(() => isPlaying.value && isAnimationEnabled.value); // Combine conditions
  ...
  const animate = (currentTime) => {
    if (!shouldAnimate.value || !analyser || audioContext?.state === 'closed') {
      console.log('Animation stopped, shouldAnimate:', shouldAnimate.value, 'analyser:', !!analyser, 'audioContext state:', audioContext?.state);
      return;
    }
  ...
  // Watchers
  watch([isPlaying, audioElement], ([newPlaying, newAudioElement]) => {
  ...
  watch(shouldAnimate, (newValue) => {
    if (newValue && analyser && audioContext?.state !== 'closed') {
      lastFrameTime = performance.now();
      animationFrameId = requestAnimationFrame(animate);
    } else if (!newValue && animationFrameId) {
      cancelAnimationFrame(animationFrameId);
      animationFrameId = null;
      clearCanvas();
    }
