...
<template>
  ...
  // Emits
  const emit = defineEmits(['play', 'add-to-queue', 'add-next', 'remove-from-library', 'toggle-play']);
  ...
  // Computed: Current track and playing status
  ...
  function onPlay() {
    if (isCurrent.value) {
      ...
      emit('play', track.trackId, section);
    }
  }
